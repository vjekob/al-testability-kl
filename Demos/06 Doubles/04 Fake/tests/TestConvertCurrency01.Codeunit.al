...
codeunit 50191 TestCovertCurrency01
{
    Subtype = Test;
    ...
    var
        ...
        ConvertCurrency: Codeunit ConvertCurrency01;
    ...
    procedure Test_06_ConversionHeader_Insert_AssignsNo()
    var
        ConversionHeader: Record ConversionHeader;
    ...
    begin
        ...
        // [GIVEN] General ledger setup
        GLSetup.Get();
        GLSetup.ConversionNos := NoSeriesCode;
        GLSetup.Modify();

        // [WHEN] Inserting a conversion header
        ConversionHeader.Insert(true);

        // [THEN] No must be assigned
        Assert.AreNotEqual('', ConversionHeader.No, 'No is not assigned');

        // [THEN] NoSeries must be assigned
        ConversionHeader.TestField(NoSeries);
    end;

    [Test]
    [TransactionModel(TransactionModel::AutoRollback)]
    procedure Test_07_ConversionHeader_No_ManualClearsNoSeries()
    var
        ConversionHeader: Record ConversionHeader;
        GLSetup: Record "General Ledger Setup";
    ...
    begin
        ...
        // [GIVEN] A conversion header
        ConversionHeader.Insert(true);

        // [WHEN] Validating No
        ConversionHeader.Validate(No, 'TEST');

        // [THEN] NoSeries must be cleared
        ConversionHeader.TestField(NoSeries, '');
    end;
}
